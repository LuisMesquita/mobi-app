...
import { setRewards, setStores, setLocation, setFranchise, setCurrentPoints } from '../actions';
...
import HomeAvailableRewardSection from '../components/HomeAvailableRewardSection';
import HomeCloseRewardSection from '../components/HomeCloseRewardSection';
  ...
  loadFavoriteStores(latitude, longitude) { 
    ...
    axios.post(`${Constants.SERVER_URL}PostEstablishmentByApp?${qs.stringify({
      ...
      .then(response => {  
        ...
        this.setState({storeId: storeData[0].Id})
...
const mapStateToProps = (state) => {
  ...
  return {
    ...
    availableRewards: state.rewards.filter((reward) => reward.Price - state.currentPoints < 0),
    closeRewards: state.rewards.filter((reward) => reward.Price - state.currentPoints > 0),
    favoriteStores: state.stores.filter((store) => store.Favorited === true),
  };
};

export default connect(mapStateToProps, { setRewards, setStores, setLocation, setFranchise, setCurrentPoints })(HomeSection);
